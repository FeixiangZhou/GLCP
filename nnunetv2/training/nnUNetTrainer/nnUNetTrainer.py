...
class nnUNetTrainer(object):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            output, output_ske, output_criticalregion, mask_criticalregion_eachbatch, localwindow_fea, localwindow_mask, prob_skeleton= self.network(data, target, train=True)

            # del data
            # l = self.loss(output, target)
            l = self.loss(output, target, output_ske, localwindow_fea, localwindow_mask, prob_skeleton, output_criticalregion, mask_criticalregion_eachbatch)
    ...
    def on_epoch_end(self):
        ...
        # self.save_checkpoint(join(self.output_folder, 'checkpoint_%d.pth' % current_epoch))   # save each epoch

        # if (current_epoch + 1) % self.save_every == 0 and current_epoch != (self.num_epochs - 1):
        #     self.save_checkpoint(join(self.output_folder, 'checkpoint_latest.pth'))

        # handle 'best' checkpointing. ema_fg_dice is computed by the logger and can be accessed like this
        self.print_to_log_file(f"ema_fg_dice: {np.round(self.logger.my_fantastic_logging['ema_fg_dice'][-1], decimals=4)}")
        if self._best_ema is None:
            self.print_to_log_file(f"_best_ema: {self._best_ema}")
        else:
            self.print_to_log_file(f"_best_ema: {np.round(self._best_ema, decimals=4)}")
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                # print('perform_actual_validation data------', data.shape)
                # print('perform_actual_validation seg------', seg.shape)
    ...
    def run_training(self, nnunet_trainer=None):
        self.on_train_start()
