...
def consistency_constraint_loss_mse(outputs_skeleton, prob_skeleton):
    ...
    # y_pred_fore_skeleton = outputs_skeleton[:, 1:]
    # y_pred_fore_skeleton = torch.max(y_pred_fore_skeleton, dim=1, keepdim=True)[0]  # C foreground channels -> 1 channel
    # y_pred_binary_skeleton = torch.cat([outputs_skeleton[:, :1], y_pred_fore_skeleton], dim=1)
    # y_prob_binary_skeleton = torch.softmax(y_pred_binary_skeleton, 1)
    # y_pred_prob_skeleton = y_prob_binary_skeleton[:, 1]  # [2,32,32,32]
    
    outputs_skeleton = F.softmax(outputs_skeleton, dim=1)
    # outputs_skeleton = outputs_skeleton[:,1]

    truncated_outputs_skeleton = outputs_skeleton.detach()  # softmax
    truncated_prob_skeleton = prob_skeleton.detach()   # softmax
    ...
    # print("mse_loss_1 truncated_prob_skeleton--------------", mse_loss_1 )
    # print("mse_loss_2 truncated_outputs_skeleton--------------", mse_loss_2)
    return Lcon
...
def weighted_bce_Loss(logits: torch.Tensor, target: torch.Tensor, weight: torch.Tensor):
    ...
    # loss = F.binary_cross_entropy_with_logits(pos_probs,  target, reduction="mean")
...
class DC_and_CE_and_CLDC_loss(nn.Module):
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor, output_ske=None, localwindow_fea=None, skeleton_gt=None, prob_ske=None, output_criticalregion=None, mask_criticalregion_eachbatch=None, t_skeletonize_flage=False):
        ...
        :return:
        """
        # print("localwindow_fea is None-----", localwindow_fea==None)
        # print("localwindow_fea----", localwindow_fea.shape)
        ...
        #     ce_loss = weighted_ce_Loss(net_output, target[:, 0].long(), output_criticalregion) \
        #         if self.weight_ce != 0 and (self.ignore_label is None or num_fg > 0) else 0

        #     iou_loss = self.iou(net_output, target[:, 0].long(), output_criticalregion)
        # else:
        #     iou_loss = 0  

        # cldice_loss = self.cldice(net_output, target, t_skeletonize_flage=t_skeletonize_flage) + 1 if self.weight_cldice != 0 else 0
        ...
        # print("****cldice_loss:", cldice_loss.item())
        # print("****cbdice_loss:", cbdice_loss.item())
        ...
        #1 ori  ce + dice
        # result = ce_loss + self.weight_dice * dc_loss


        #2 bce_loss_criticalregion 
        # result = ce_loss + self.weight_dice * dc_loss  +  bce_loss_criticalregion 


        #3 ce_loss_ske   
        # result = ce_loss + self.weight_dice * dc_loss  +  ce_loss_ske


        #4 bce_loss_criticalregion + ce_loss_ske 
        # result =  ce_loss + self.weight_dice * dc_loss  +  bce_loss_criticalregion + ce_loss_ske


        #5 bce_loss_criticalregion + ce_loss_ske  + con_losss
        # result =  ce_loss + self.weight_dice * dc_loss  +  bce_loss_criticalregion + ce_loss_ske +  0.5 * con_loss


        # bce_loss_criticalregion no dice  + ce_loss_ske+  ce_loss_refine
        result = ce_loss + self.weight_dice * dc_loss  + bce_loss_criticalregion + ce_loss_ske + 0.5* ce_loss_refine 
       

        # refine_fullres
        # result =   ce_loss + self.weight_dice * dc_loss  + bce_loss_criticalregion + ce_loss_ske +  0.5 * con_loss + 0.5 * ce_loss_refine


        #cldice
        # result = 2* ce_loss + 1* dc_loss  + cldice_loss   

        #cbdice
        # result = 2*ce_loss +  1* dc_loss  + cbdice_loss   
        # result = 3*ce_loss +  1* dc_loss  + 2*cbdice_loss    #in cbdice papers
