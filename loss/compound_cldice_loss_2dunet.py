    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor, output_ske=None, localwindow_fea=None, skeleton_gt=None, prob_ske=None, output_criticalregion=None, mask_criticalregion_eachbatch=None, t_skeletonize_flage=False):
        ...
        if localwindow_fea is not None:
            if (len(localwindow_fea) > 0):
                ce_loss_refine = self.ce(localwindow_fea, target[:, 0].long())
        ...
        if output_ske is not None:
            ...
            dc_loss_ske = self.dc(output_ske, skel_true) + 1
            ce_loss_ske = ce_loss_ske + dc_loss_ske
        else:
            ce_loss_ske=0
        ...
        #bce_loss_criticalregion + ce_loss_ske
        # result =  ce_loss + self.weight_dice * dc_loss  +  bce_loss_criticalregion + ce_loss_ske

        #bce_loss_criticalregion + ce_loss_ske  + con_los
        ...
        # refine_fullres
        # result =   ce_loss + dc_loss  + bce_loss_criticalregion + ce_loss_ske +  0.5 * con_loss + 0.5 * ce_loss_refine


        #cldice
        # result = ce_loss + 1 * dc_loss  + cldice_loss   

        #cbdice
        # result = ce_loss +  1* dc_loss  + cbdice_loss   
